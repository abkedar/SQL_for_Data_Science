...
/* The idea is you want to give MORE weight to the MORE RECENT event than the past events.
...
SELECT 
    ...
    WHERE d2.event_date = d1.event_date - INTERNAL '1' day
    ) +
    (SELECT ROUND(avg_temp,2) * 0.333
    FROM daily_avg_temp d3
    WHERE d3.event_date = d1.event_date - INTERNAL '2' day
    ) +
    (SELECT ROUND(avg_temp, 2) * 0.167
    FROM daily_avg_temp d4
    WHERE d4.event_date = d1.event_date - INTERNAL '3' day
