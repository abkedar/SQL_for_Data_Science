...
/* create new table with partition table (with pattition key) */
...
/* If we check iot_measurement table > Partitions > we will see No Nodes yet, 
because none has been created */

/* Create partition Nodes and add to the original table */

CREATE TABLE iot_measurement_week1_2019 PARTITION BY (iot_measurement)
FOR values from ('2019-01-01') TO ('2019-01-08');

CREATE TABLE iot_measurement_week2_2019 PARTITION BY (iot_measurement)
FOR values from ('2019-01-08') TO ('2019-01-015');
...
CREATE TABLE product_clothing PARTITION BY (products)
	FOR VALUES IN('casual_clothing', 'business_attire', 'formal_clothing');
	
CREATE TABLE product_electronics PARTITION BY (products)
	FOR VALUES IN('mobile_phones', 'tablets', 'laptop_computers');
	
CREATE TABLE product_kitchen PARTITION BY (products)
...
CREATE TABLE customer_interaction_1 PARTITION BY (customer_interaction)
FOR VALUES WITH (MODULUS 5, REMAINDER 0);

CREATE TABLE customer_interaction_2 PARTITION BY (customer_interaction)
FOR VALUES WITH (MODULUS 5, REMAINDER 1);

CREATE TABLE customer_interaction_3 PARTITION BY (customer_interaction)
FOR VALUES WITH (MODULUS 5, REMAINDER 2);

CREATE TABLE customer_interaction_4 PARTITION BY (customer_interaction)
FOR VALUES WITH (MODULUS 5, REMAINDER 3);

CREATE TABLE customer_interaction_5 PARTITION BY (customer_interaction)
